...
public class ServerService implements Runnable{
	...
	public void run() {
		...
		//处理请求
		//请求的是servlet还是静态资源，那如何判断是动态资源呢？如果是要映射到动态资源，则肯定会配置到对应项目的web.xml文件中
		...
		/*
		 * 处理动态资源
		 * 我的规则：所有的动态资源处理->servlet代码必须放到当前项目下面的bin目录中
		 */
		//要动态的加载当前访问的这个项目下的bin目录中的类
		URLClassLoader loader=null; //类加载器
		URL classPath=null; //要加载的这个类的地址
		//url=/snacknet/login?id=123&name=yc
		url=url.substring(1); //去掉最前面的/  -> snacknet/login?id=123&name=yc
